...
class SimCLRLoss(nn.Module):
	...
	def forward(self, features: torch.Tensor, labels: Optional[torch.Tensor] = None) ->\
		...
		numerator_mask = torch.mul(numerator_mask, no_self_similarity_mask)
		...
		print(torch.div(torch.mul(all_similarities, numerator_mask).sum(1), 
			torch.mul(all_similarities, denominator_mask).sum(1)))
